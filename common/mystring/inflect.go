...
var lookalikes map[string]*regexp.Regexp = map[string]*regexp.Regexp{
	// "A":  regexp.MustCompile(`À|Á|Ạ|Ả|Ã|Â|Ấ|Ầ|Ẩ|Ẫ|Ậ|Ă|Ắ|Ằ|Ẳ|Ẵ|Ặ|Ä|Å`),
	// "AE": regexp.MustCompile(`Æ`),
	// "C":  regexp.MustCompile(`Ç`),
	// "E":  regexp.MustCompile(`È|É|Ẻ|Ẽ|Ẹ|Ê|Ề|Ế|Ể|Ễ|Ệ|Ë`),
	// "I":  regexp.MustCompile(`Ì|Í|Ỉ|Ĩ|Ị|Î|Ï`),
	// "N":  regexp.MustCompile(`Ñ`),
	// "O":  regexp.MustCompile(`Ò|Ó|Ỏ|Õ|Ọ|Ô|Ố|Ồ|Ổ|Ỗ|Ộ|Ơ|Ớ|Ờ|Ở|Ỡ|Ợ|Ö|Ø`),
	// "U":  regexp.MustCompile(`Ù|Ú|Ủ|Ũ|Ụ|Ư|Ứ|Ừ|Ử|Ữ|Ự|Û|Ü`),
	// "Y":  regexp.MustCompile(`Ý|Ỳ|Ỷ|Ỹ|Ỵ`),
	// "ss": regexp.MustCompile(`ß`),
	// "D":  regexp.MustCompile(`Đ`),
	// "a":  regexp.MustCompile(`à|á|ả|ã|ạ|â|ấ|ầ|ẩ|ẫ|ậ|ă|ắ|ằ|ẳ|ẵ|ặ|ä|å`),
	// "ae": regexp.MustCompile(`æ`),
	// "c":  regexp.MustCompile(`ç`),
	// "e":  regexp.MustCompile(`è|é|è|ẽ|ẻ|ê|ế|ề|ể|ễ|ệ|ë`),
	// "i":  regexp.MustCompile(`ì|í|ỉ|ĩ|ị|î|ï`),
	// "n":  regexp.MustCompile(`ñ`),
	// "o":  regexp.MustCompile(`ò|ó|ỏ|õ|ọ|ô|ố|ồ|ổ|ỗ|ộ|ö|ø`),
	// "u":  regexp.MustCompile(`ù|ú|ủ|ũ|ụ|ư|ứ|ừ|ử|ự|ữ|û|ü|ū|ŭ|ů|ű|ų`),
	// "y":  regexp.MustCompile(`ý|ỳ|ỹ|ỷ|ỵ|ÿ`),
	// "d":  regexp.MustCompile(`đ`),
	"A":  regexp.MustCompile(`Ä|Å|Α|А|Ą|Ā|Ά|Á|À|Ả|Ã|Ạ|Â|Ấ|Ầ|Ẩ|Ẫ|Ậ|Ă|Ắ|Ằ|Ẳ|Ẵ|Ặ`),
	"a":  regexp.MustCompile(`ä|å|α|а|ą|ā|ά|á|à|ả|ã|ạ|â|ấ|ầ|ẩ|ẫ|ậ|ă|ắ|ằ|ẳ|ẵ|ặ`),
	"AE": regexp.MustCompile(`Æ`),
	"ae": regexp.MustCompile(`æ`),
	"B":  regexp.MustCompile(`Β|Б`),
	"b":  regexp.MustCompile(`β|б`),
	"C":  regexp.MustCompile(`Ç|Č|Ć|Ц`),
	"c":  regexp.MustCompile(`ç|č|ć|ц|©`),
	"Ch": regexp.MustCompile(`Ч`),
	"ch": regexp.MustCompile(`ч`),
	"D":  regexp.MustCompile(`Δ|Д|Ď|Đ`),
	"d":  regexp.MustCompile(`δ|д|ď|ð|đ`),
	"E":  regexp.MustCompile(`Ë|Ε|Е|Ě|Ę|Ē|Έ|Э|É|È|Ẻ|Ẽ|Ẹ|Ê|Ế|Ề|Ể|Ễ|Ệ`),
	"e":  regexp.MustCompile(`ë|ε|е|ě|ę|ē|έ|э|é|è|ẻ|ẽ|ẹ|ê|ế|ề|ể|ễ|ệ`),
	"F":  regexp.MustCompile(`Φ|Ф`),
	"f":  regexp.MustCompile(`φ|ф`),
	"G":  regexp.MustCompile(`Γ|Ğ|Г|Ґ|Ģ`),
	"g":  regexp.MustCompile(`γ|ğ|г|ґ|ģ`),
	"H":  regexp.MustCompile(`Η|Ή|Х`),
	"h":  regexp.MustCompile(`η|ή|х`),
	"I":  regexp.MustCompile(`Î|Ï|Ι|İ|І|Ī|Ί|Ϊ|И|Í|Ì|Ỉ|Ĩ|Ị`),
	"i":  regexp.MustCompile(`î|ï|ι|ı|і|ī|ί|ϊ|и|í|ì|ỉ|ĩ|ị|ΐ`),
	"J":  regexp.MustCompile(`Й`),
	"j":  regexp.MustCompile(`й`),
	"K":  regexp.MustCompile(`Κ|К|Ķ`),
	"k":  regexp.MustCompile(`κ|к|ķ`),
	"L":  regexp.MustCompile(`Λ|Л|Ł|Ļ`),
	"l":  regexp.MustCompile(`λ|л|ł|ļ`),
	"M":  regexp.MustCompile(`Μ|М`),
	"m":  regexp.MustCompile(`μ|м`),
	"N":  regexp.MustCompile(`Ñ|Ň|Ń|Ņ|Ν|Н`),
	"n":  regexp.MustCompile(`ñ|ň|ń|ņ|ν|н`),
	"O":  regexp.MustCompile(`Ö|Ő|Ø|Ο|Ό|О|Ó|Ò|Ỏ|Õ|Ọ|Ô|Ố|Ồ|Ổ|Ỗ|Ộ|Ơ|Ớ|Ờ|Ở|Ỡ|Ợ`),
	"o":  regexp.MustCompile(`ö|ő|ø|ο|ό|о|ó|ò|ỏ|õ|ọ|ô|ố|ồ|ổ|ỗ|ộ|ơ|ớ|ờ|ở|ỡ|ợ`),
	"P":  regexp.MustCompile(`Π|П`),
	"p":  regexp.MustCompile(`π|п`),
	"PS": regexp.MustCompile(`Ψ`),
	"ps": regexp.MustCompile(`ψ`),
	"R":  regexp.MustCompile(`Ρ|Р|Ř`),
	"r":  regexp.MustCompile(`ρ|р|ř`),
	"S":  regexp.MustCompile(`Σ|Ş|Š|Ś|С`),
	"s":  regexp.MustCompile(`σ|ş|š|ś|с|ς`),
	"Sh": regexp.MustCompile(`Ш|Щ`),
	"sh": regexp.MustCompile(`ш|щ`),
	"ss": regexp.MustCompile(`ß`),
	"T":  regexp.MustCompile(`Τ|Т|Ť`),
	"t":  regexp.MustCompile(`τ|т|ť`),
	"TH": regexp.MustCompile(`Þ`),
	"th": regexp.MustCompile(`þ`),
	"U":  regexp.MustCompile(`Û|Ů|Ū|Ü|Ű|У|Ú|Ù|Ủ|Ũ|Ụ|Ư|Ứ|Ừ|Ử|Ữ|Ự`),
	"u":  regexp.MustCompile(`û|ů|ū|ü|ű|у|ú|ù|ủ|ũ|ụ|ư|ứ|ừ|ử|ữ|ự`),
	"V":  regexp.MustCompile(`В`),
	"v":  regexp.MustCompile(`в`),
	"W":  regexp.MustCompile(`Ω|Ώ`),
	"w":  regexp.MustCompile(`ω|ώ`),
	"X":  regexp.MustCompile(`Χ`),
	"x":  regexp.MustCompile(`χ`),
	"Y":  regexp.MustCompile(`Ϋ|Υ|Ύ|Ы|Ý|Ỳ|Ỷ|Ỹ|Ỵ`),
	"y":  regexp.MustCompile(`ϋ|υ|ύ|ы|ý|ỳ|ỷ|ỹ|ỵ|ÿ|ΰ`),
	"Yo": regexp.MustCompile(`Ё`),
	"yo": regexp.MustCompile(`ё`),
	"Yu": regexp.MustCompile(`Ю`),
	"yu": regexp.MustCompile(`ю`),
	"Ya": regexp.MustCompile(`Я`),
	"ya": regexp.MustCompile(`я`),
	"Ye": regexp.MustCompile(`Є`),
	"ye": regexp.MustCompile(`є`),
	"Yi": regexp.MustCompile(`Ї`),
	"yi": regexp.MustCompile(`ї`),
	"Z":  regexp.MustCompile(`Ζ|З|Ž|Ź|Ż`),
	"z":  regexp.MustCompile(`ζ|з|ž|ź|ż`),
	"Zh": regexp.MustCompile(`Ж`),
	"zh": regexp.MustCompile(`ж`),
}
